    ...
    >
      <div className="w-full max-w-7xl mx-auto text-center">
        ...
        <div className="bg-gray-50 rounded-xl sm:rounded-2xl lg:rounded-[2rem] p-3 sm:p-4 md:p-6 lg:p-10 mb-6 md:mb-8 relative overflow-hidden shadow-2xl w-full mx-auto">
          ...
          <div className="flex items-center justify-between mb-6 sm:mb-8 lg:mb-10">
            <div className="flex items-center space-x-2 sm:space-x-3">
              ...
              <span className="font-semibold text-gray-800 text-sm sm:text-base lg:text-lg">Pallet Ross</span>
            </div>
          ...
          {/* Cards Container */}
          <div className="relative h-40 sm:h-48 md:h-56 lg:h-64 mb-6 sm:mb-8 lg:mb-12 flex items-center justify-center overflow-hidden">
            {cards.map((card, index) => (
              <div
                key={card.id}
                className="absolute top-1/2 left-1/2 w-24 h-32 xs:w-32 xs:h-40 sm:w-40 sm:h-52 md:w-44 md:h-56 lg:w-48 lg:h-64 rounded-lg sm:rounded-xl lg:rounded-2xl shadow-lg sm:shadow-xl lg:shadow-2xl transition-all duration-1200 ease-out"
                style={{
                  background: card.bgImage,
                  transform: isVisible
                    ? `translate(-50%, -50%) rotate(${card.rotation}) translateX(${card.translateX * 0.3}px) translateY(0px)`
                ...
                <div className="w-full h-full bg-white rounded-lg sm:rounded-xl lg:rounded-2xl p-1 sm:p-2 lg:p-4 m-0.5 sm:m-1 shadow-inner">
                  <div className="w-full h-full bg-gradient-to-br from-gray-50 to-gray-100 rounded-md sm:rounded-lg lg:rounded-xl flex flex-col items-center justify-center relative overflow-hidden">
                    ...
                    <div className="absolute inset-1 sm:inset-2 bg-white rounded-sm sm:rounded-md lg:rounded-lg shadow-sm flex flex-col items-center justify-center">
                      <div className="text-xs xs:text-sm sm:text-lg lg:text-2xl font-bold text-gray-700 mb-0.5 sm:mb-1">{card.content}</div>
                      <div className="text-[8px] xs:text-[10px] sm:text-xs text-gray-400 uppercase tracking-wide">Collection</div>
            ...
            {/* Speech Bubbles */}
            <div
              className={`absolute top-4 sm:top-6 lg:top-8 left-1/4 bg-blue-500 text-white px-2 sm:px-3 lg:px-4 py-1 sm:py-1.5 lg:py-2 rounded-full text-[10px] xs:text-xs sm:text-sm font-semibold transition-all duration-800 shadow-lg ${
            ...
            <div
              className={`absolute top-6 sm:top-8 lg:top-12 right-1/4 bg-emerald-500 text-white px-2 sm:px-3 lg:px-4 py-1 sm:py-1.5 lg:py-2 rounded-full text-[10px] xs:text-xs sm:text-sm font-semibold transition-all duration-800 shadow-lg ${
                isVisible ? "opacity-100 translate-y-0 scale-100" : "opacity-0 translate-y-6 scale-75"
              }`}
          ...
          {/* Buttons */}
          <div className="flex flex-col xs:flex-row items-center justify-center space-y-3 xs:space-y-0 xs:space-x-4 sm:space-x-6 px-2 sm:px-4">
            <button className="bg-black text-white px-4 sm:px-6 lg:px-8 py-2.5 sm:py-3 lg:py-4 rounded-full hover:bg-gray-800 transition-all duration-300 font-semibold shadow-lg hover:shadow-xl transform hover:scale-105 text-xs sm:text-sm lg:text-base w-full xs:w-auto">
              Join for $9/month
            </button>
            <button className="text-gray-600 hover:text-gray-800 transition-colors font-medium underline text-xs sm:text-sm lg:text-base">
              Read more
            </button>
          </div>
        </div>
        ...
        <div className="text-left pl-2 sm:pl-4">
          <h2 className="text-white text-lg sm:text-xl lg:text-2xl xl:text-3xl font-bold mb-1 sm:mb-2">Pallet ross</h2>
          <p className="text-gray-400 text-xs sm:text-sm lg:text-base xl:text-lg">A place to display your masterpiece</p>
        </div>
      </div>
    </div>
  );
